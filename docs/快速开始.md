...
如果你有一个视频，想直接得到模型，可以使用本项目集成的全自动脚本。它会自动完成抽帧、位姿计算、去畸变和训练，并**自动检测 COLMAP 是否成功**，失败则报错停止。

```bash
# 用法: ./full_pipeline.sh <视频路径> <场景名称> [可选:迭代次数]
./full_pipeline.sh ~/projects/Sparse2DGS/video.mp4 my_video 7000
```
